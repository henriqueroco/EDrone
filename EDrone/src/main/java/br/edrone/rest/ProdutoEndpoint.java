...
{
   @PersistenceContext(unitName = "EDrone-persistence-unit")
   ...
   {
      TypedQuery<Produto> findByIdQuery = em.createQuery("SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.categoria WHERE p.id = :entityId ORDER BY p.id", Produto.class);
   ...
   {
      TypedQuery<Produto> findAllQuery = em.createQuery("SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.categoria ORDER BY p.id", Produto.class);
